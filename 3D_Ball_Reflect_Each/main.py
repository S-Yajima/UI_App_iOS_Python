...
# カメラの位置と注視点を変更する
def change_view_schedule(main_view, camera_pos, lookat_pos, lock):
    if isinstance(main_view, BaseView) is False:
        return
    if not type(camera_pos) is list:
        return
    if len(camera_pos) < 3:
        return
    if not type(lookat_pos) is list:
        return
    if len(lookat_pos) < 3:
        return
...
def reflect_sphere_with_wall(sphere, wall_V_n, wall_x, wall_y, wall_z):
    is_reflect = False
    ...
    if distance - sphere.radius <= proceed_length and proceed_cos_theta < 0:
        ...
        # 進行ベクトルを更新する
        sphere.proceed_vector[0] = proceed_vector[0] + (2 * r_proceed_dot_n_value * wall_V_n[0])
        sphere.proceed_vector[1] = proceed_vector[1] + (2 * r_proceed_dot_n_value * wall_V_n[1])
        sphere.proceed_vector[2] = proceed_vector[2] + (2 * r_proceed_dot_n_value * wall_V_n[2])
...
def move_sphere_schedule(main_view, sphere, lock):
    ...
    is_reflect = reflect_sphere_with_wall(
    ...
    # 球同士の衝突
    if is_reflect is False:
        for other_sphere in main_view.spheres:
            if id(sphere) == id(other_sphere):
                continue

            if reflect_sphere_with_sphere(
                    sphere, other_sphere) is True:
                is_reflect = True
                break
...
# 球同士の接触の判定と反射
# 球同士の接触を判別したら、接点まで移動する
# 接触なし:False 接触あり:True
def reflect_sphere_with_sphere(sphere, other_sphere):
    # 接触を判定する
    is_reflect = False

    # 自身の球の進行距離を算出する
    proceed_v = sphere.proceed_vector
    proceed_length = sqrt(proceed_v[0] ** 2 + proceed_v[1] ** 2 + proceed_v[2] ** 2)
    ...
    center_length = sqrt(center_v[0] ** 2 + center_v[1] ** 2 + center_v[2] ** 2)

    # 接触を判定する
    if center_length < (sphere.radius + other_sphere.radius):
        # 中心同士のベクトルを正規化する
        center_v_n = [
            center_v[0] / center_length,
            center_v[1] / center_length,
            center_v[2] / center_length]
        # 進行ベクトルごとに近づく円の中心までの距離を算出する
        approach_length = dot_product(proceed_v, center_v_n)

        if approach_length <= 0.0:
            # このまま進むと他の円と鋭角で接触する場合
            # 球の接点まで移動する
            rate = 1
            if center_length != 0:
                rate = abs(approach_length) / center_length
            sphere.set_center(
                sphere.center[0] + (proceed_v[0] * rate),
                sphere.center[1] + (proceed_v[1] * rate),
                sphere.center[2] + (proceed_v[2] * rate))
            ...
            r_proceed_v = [
                ...
                -1 * proceed_v[1],
                -1 * proceed_v[2]]
            r_proceed_dot_n_value = dot_product(r_proceed_v, center_v_n)

            # Todo: 配列要素一つ一つに値を代入する方法を試す
            sphere.proceed_vector[0] = proceed_v[0] + (2 * r_proceed_dot_n_value * center_v_n[0])
            sphere.proceed_vector[1] = proceed_v[1] + (2 * r_proceed_dot_n_value * center_v_n[1])
            sphere.proceed_vector[2] = proceed_v[2] + (2 * r_proceed_dot_n_value * center_v_n[2])

            is_reflect = True
        else:
            # Todo
            # 進行ベクトルの後方から他の球が接触する場合
            # 特に動作上違和感を感じないため何も処理を行わない
            # 必要を感じた時点で実装を検討する。
            pass
...
def display_schedule(main_view, lock):
    ...
    if main_view.on_screen is True:
        t = threading.Timer(0.02, display_schedule, args=[main_view, lock])
        t.start()
...
if __name__ == '__main__':
    ...
    sphere_2.set_center(-180, 0, -250)
    sphere_2.face_color = 'orange'
    main_view.add_sphere(sphere_2)

    # 三つ目の球
    sphere_3 = Sphere(80, is_hide=True)
    sphere_3.set_center(180, 0, -250)
    sphere_3.face_color = 'green'
    ...
    # 視点移動 アップ
    delay += 2.0
    tv1 = threading.Timer(delay, change_view_schedule, args=[main_view, [-180, -50, -200], [0, 0, -250], lock])
    tv1.start()

    delay += 1.0
    tv2 = threading.Timer(delay, change_view_schedule, args=[main_view, [180, -50, -200], [0, 0, -250], lock])
    tv2.start()

    # 視点を通常に戻す
    delay += 1.0
    tv5 = threading.Timer(delay, change_view_schedule, args=[main_view, [0, 0, 150], [0, 0, -250], lock])
    tv5.start()

    # 球を落下させる
    delay += 4.0
    t4 = threading.Timer(delay, fall_sphere_schedule, args=[main_view, sphere, [-7, 3, -1], lock])
    t4.start()

    delay += 0.5
    t5 = threading.Timer(delay, fall_sphere_schedule, args=[main_view, sphere_2, [1, 1, -7], lock])
    t5.start()

    delay += 0.5
    t6 = threading.Timer(delay, fall_sphere_schedule, args=[main_view, sphere_3, [1, -7, 1], lock])
    t6.start()
    ...
    # 視点切替
    delay += 38.0
    tv6 = threading.Timer(delay, change_view_schedule, args=[main_view, [150, 150, 150], [0, 0, -250], lock])
    tv6.start()

    delay += 10.0
    tv7 = threading.Timer(delay, change_view_schedule, args=[main_view, [-100, -100, 150], [0, 0, -250], lock])
    tv7.start()

    delay += 10.0
    tv8 = threading.Timer(delay, change_view_schedule, args=[main_view, [0, 0, 150], [0, 0, -250], lock])
    tv8.start()
