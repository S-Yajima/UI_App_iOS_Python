...
class BaseView(ui.View):
	...
	def set_sphere_color(self, color, light_inner, light_length, brightness=60000, specular=1, specular_inner=1):
		...
		# 拡散反射
		# (光源光量 * cosθ) / 距離の二乗
		diffused_light_value = (-1 * brightness * light_inner) / light_length
		...
		specular_light_value = 0
		if specular_inner < 0:
			specular_light_value = (brightness * ((-1*specular_inner)**specular)) / light_length
		
		value = diffused_light_value + specular_light_value
