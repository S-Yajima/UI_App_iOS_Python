...
class BaseView(ui.View):
	...
	def draw(self):
		...
		for character in self.characters:
			...
			# 影の位置を算出する為の座標を確保する
			shadow_contours = copy.deepcopy(contours)
			# 影を描画する
			self.draw_shadow(shadow_contours)
...
# font x軸に指定の角度まで10度ずつ回転させる
# 一周するまで本関数が繰り返し呼ばれる
def rotate_x_font_schedule(main_view, character, roled_angle, goal_angle):
	if isinstance(main_view, BaseView) is False:
		return
	if isinstance(character, Character) is False:
		return
	...
	character.rotate_angle_x += add_angle
	roled_angle += add_angle
	if character.rotate_angle_x >= 360:
		character.set_angle_x(0)
	
	if main_view.on_screen is True and goal_angle > roled_angle:
		t = threading.Timer(0.02, rotate_x_font_schedule, args=[main_view, character, roled_angle, goal_angle])
		t.start()
...
# 空の色を変える
def change_sky_schedule(main_view, color_list, index):
	if isinstance(main_view, BaseView) is False:
		return
	
	if len(color_list) > index:
		main_view.background_color = color_list[index]
	index += 1
		
	if main_view.on_screen is True and len(color_list) > index:
		t = threading.Timer(0.50, change_sky_schedule, args=[main_view, color_list, index])
		t.start()
...
if __name__ == '__main__':
	...
	main_view.name = 'へのへのもへじ(文字の形の影付)'
	...
	# 空の色変更リスト
	sky_color_list_1 = [
		'#d2e3e6', '#e5e6dc', '#e6e2dc', 
		'#e6d9c8', '#e6d1b4', '#e6c8a0']
	sky_color_list_2 = [
		'#e6bf8c', '#e68cbf', '#bf8ce6', 
		'#b36ee6', '#7446e6', '#4200e6']
	...
	delay = 1.5
	# 空の色を変える
	tc1 = threading.Timer(delay, change_sky_schedule, args=[main_view, sky_color_list_1, 0])
	tc1.start()
	
	# _文字をX軸に回転させる
	for character in character_list:
		tr1 = threading.Timer(delay, rotate_x_font_schedule, args=[main_view, character, 0, 360])
		tr1.start()
		delay += 0.5
	...
	# 文字をX軸に回転させる
	for character in character_list:
		tr2 = threading.Timer(delay, rotate_x_font_schedule, args=[main_view, character, 0, 320])
		tr2.start()
		delay += 0.5
	...
	for i in range(5):
		...
		# 文字をX軸に回転させる
		for character in character_list:
			tr5 = threading.Timer(delay, rotate_x_font_schedule, args=[main_view, character, 0, 300])
			tr5.start()
			delay += 0.5
	
	# 空の色を変える
	tc2 = threading.Timer(35.0, change_sky_schedule, args=[main_view, sky_color_list_2, 0])
	tc2.start()
	
	delay += 0.1
	
	# 視点移動　上昇
	tv3 = threading.Timer(40.0, change_view_y_schedule, args=[main_view, -200, -4, lock])
	tv3.start()
	
	# 視点移動　下降
	tv4 = threading.Timer(60.0, change_view_y_schedule, args=[main_view, 0, 4, lock])
	tv4.start()
