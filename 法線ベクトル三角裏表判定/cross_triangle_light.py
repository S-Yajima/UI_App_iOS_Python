...
class BaseView(ui.View):
    ...
    def dot_product(self, a_x, a_y, a_z,
        ...
        # 法線ベクトルの正規化を実行する
        normal_AB_AC = [
            cross_AB_AC[0] / length_AB_AC,
            cross_AB_AC[1] / length_AB_AC,
            cross_AB_AC[2] / length_AB_AC]
        ...
        # 焦点距離までのベクトルの正規化を行う
        normal_scr = [
            scr_vector[0] / length_scr,
            scr_vector[1] / length_scr,
            scr_vector[2] / length_scr]

        # 視点のベクトルとAB,ACの外積との内積を求める
        inner = normal_scr[0] * normal_AB_AC[0] + \
                normal_scr[1] * normal_AB_AC[1] + \
                normal_scr[2] * normal_AB_AC[2]
    ...
    def set_ui_color(self, color, inner):
        if color == 'blue':
            ...
            ui.set_color(((inner * -1), 0.3, 0.3, 1.0))
        elif color == 'yellow':
