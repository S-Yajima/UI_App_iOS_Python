...
def change_view_round_schedule(main_view, angle, goal_angle, lock):
    ...
    main_view.camera_z = round * sin(radians(angle)) + (-150)
    main_view.set_camera_coodinate_vector()
    # main_view.set_needs_display()
...
def display_schedule(main_view, lock):
    ...
    if main_view.on_screen is True:
        t = threading.Timer(0.010, display_schedule, args=[main_view, lock])
        t.start()
...
if __name__ == '__main__':
    ...
    main_view.name = '折り紙_風車_視点移動'
    main_view.background_color = 'lightblue'

    # 視点移動で使用するカメラの位置を設定する
    main_view.set_camera_position(0, -8000, 100)
    # 視点移動で使用する注視点を設定する
    main_view.set_lookat_position(0, 50, -150)
    # ビュー座標XYZ軸のベクトルを算出する
    main_view.set_camera_coodinate_vector()

    # 水面
    surface = Square(-200, -300, 0, -200, 0, 0, 200, 0, 0, 200, -300, 0)
    surface.set_center(0, 0, 0)
    surface.set_shift(0, 250, 0)
    surface.set_scale(1)
    ...
    delay += 0.5
    tv1 = threading.Timer(delay, change_view_y_schedule, args=[main_view, 50, 80, lock])
    tv1.start()
    ...
    delay += 4
    tv3 = threading.Timer(delay, change_view_y_schedule, args=[main_view, -200, -5, lock])
    tv3.start()
    ...
    delay += 0.5
    tv2 = threading.Timer(delay, change_view_round_schedule, args=[main_view, 90, 90 + 360 * 5, lock])
    tv2.start()
